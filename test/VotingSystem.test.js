...
contract("VotingSystem", (accounts) => {
  ...
  it("should allow a voter to commit a vote", async () => {
    ...
    await proposalManager.raiseProposal(title, description, budget, solution, {
      from: unitProposer,
    });
...
describe("Vote Commitment", () => {
  ...
  it("should reject commit votes from units with revoked voting rights", async () => {
    ...
    try {
      ...
      assert(
        error.message.includes("Unit does not have voting rights"),
        "Expected 'Unit does not have voting rights' error"
      );
    }
  });
});
