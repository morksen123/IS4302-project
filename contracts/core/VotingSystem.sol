...
contract VotingSystem {
    ...
    event ProposalContractSet(address proposalManager);
    event VoteCommitted(uint256 proposalId, address indexed voter, bytes32 commitHash);
    event VoteRevealed(uint256 proposalId, address indexed voter, VotingStorage.VoteOption choice);
    event VotingStarted();
    event VotingClosed();

    constructor(address _votingStorage, address _unitManager) {
        require(_votingStorage != address(0), "Invalid voting storage address");
        require(_unitManager != address(0), "Invalid unit manager address");
    ...
    modifier votingOpen(uint256 proposalId) {
        require(
            proposalManager.getProposal(proposalId).status == DataTypes.ProposalStatus.VotingOpen,
            "Voting is not open"
        );
        _;
    }

    function setProposalContract(address _proposalManager) external onlyOwner {
        require(proposalManager == ProposalManager(address(0)), "Proposal manager already set");
        require(_proposalManager != address(0), "Invalid proposal manager address");

        proposalManager = ProposalManager(_proposalManager);
        emit ProposalContractSet(_proposalManager);
    }

    function startVoting() external onlyOwner {
        DataTypes.Proposal[] memory proposals = proposalManager.getAllProposals();

        for (uint256 i = 0; i < proposals.length; i++) {
            if (proposals[i].status == DataTypes.ProposalStatus.Submitted) {
                proposalManager.updateProposalStatus(i, DataTypes.ProposalStatus.VotingOpen);
            }
        }
    ...
    {
        require(votingStorage.getUserCommit(msg.sender, proposalId).status == VotingStorage.VoteStatus.None, "Vote already committed");
    ...
    function revealVote(uint256 proposalId, uint256 choice, string memory secret)
        external
        validProposal(proposalId)
        votingOpen(proposalId)
    {
        ...
        emit VoteRevealed(proposalId, msg.sender, voteChoice);
    }
    ...
    function closeVoting() external onlyOwner {
        ...
        for (uint256 i = 0; i < proposals.length; i++) {
            if (proposals[i].status == DataTypes.ProposalStatus.VotingOpen) {
                proposalManager.updateProposalStatus(i, DataTypes.ProposalStatus.VotingClosed);
            }
        }
    ...
    function getProposal(uint256 proposalId) external view validProposal(proposalId) returns (DataTypes.Proposal memory) {
        return votingStorage.getProposal(proposalId);
    }
    ...
    // Helper function to convert a uint choice to VoteOption enum
    function _convertToVoteOption(uint256 choice) private pure returns (VotingStorage.VoteOption) {
        if (choice == 1) return VotingStorage.VoteOption.For;
        if (choice == 2) return VotingStorage.VoteOption.Against;
        if (choice == 3) return VotingStorage.VoteOption.Abstain;
        revert("Invalid vote choice");
    }

    // Helper function to update votes in the proposal
    function _updateVotes(uint256 proposalId, VotingStorage.VoteOption choice) private {
        if (choice == VotingStorage.VoteOption.For) {
            proposalManager.incrementVotes(proposalId, 1, 0, 0);
        } else if (choice == VotingStorage.VoteOption.Against) {
            proposalManager.incrementVotes(proposalId, 0, 1, 0);
        } else if (choice == VotingStorage.VoteOption.Abstain) {
            proposalManager.incrementVotes(proposalId, 0, 0, 1);
        }
    }
}
